...
void initServerConnection(char *type, char *destination){
    if (strcmp(type, "local") == 0){
        ...
        getaddrinfo("localhost", destination, &addrinfo, &info);
...
int main(int argc, char *argv[]){
    if (argc != 4){
        printf("USAGE: ./client [name] [type] [destination]");
        return 1;
    }
    name = argv[1];
    char *type = argv[2];
    ...
    signal(SIGINT, quit);
    initServerConnection(type, destination);
